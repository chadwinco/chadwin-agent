...
- Confirm these artifacts exist:
  - `<DATA_ROOT>/companies/US/PEP/reports/<YYYY-MM-DD>/report.md`
  - `<DATA_ROOT>/companies/US/PEP/reports/<YYYY-MM-DD>/valuation/inputs.yaml`
  - `<DATA_ROOT>/companies/US/PEP/reports/<YYYY-MM-DD>/valuation/outputs.json`
...
- Confirm data files are created in `<DATA_ROOT>/companies/US/AAPL/data/`.
- Confirm `<DATA_ROOT>/companies/US/AAPL/data/filings/earnings-call-*.md` exists.
...
- Confirm data files are created in `<DATA_ROOT>/companies/<EXCHANGE_COUNTRY>/<IDENTIFIER>/data/`.
- Confirm `<DATA_ROOT>/companies/<EXCHANGE_COUNTRY>/<IDENTIFIER>/reports/<YYYY-MM-DD>/valuation/inputs.yaml` exists.

## 4. Missing data scenario
- Temporarily rename one required CSV in `<DATA_ROOT>/companies/<EXCHANGE_COUNTRY>/<TICKER>/data/financial_statements/annual/`.
...
## 6. Valuation sanity
- Adjust discount rates in `<DATA_ROOT>/companies/<EXCHANGE_COUNTRY>/<TICKER>/reports/<YYYY-MM-DD>/valuation/inputs.yaml`.
- Re-run `$run-llm-workflow` for the same ticker/date.
- Confirm value-per-share shifts in `<DATA_ROOT>/companies/<EXCHANGE_COUNTRY>/<TICKER>/reports/<YYYY-MM-DD>/valuation/outputs.json`.
...
- `python chadwin-codex/.agents/skills/fetch-us-investment-ideas/scripts/fetch_us_investment_ideas.py --limit 5 --output <DATA_ROOT>/idea-screens/<YYYY-MM-DD>/ideas.json`
- Confirm `<DATA_ROOT>/idea-screens/company-ideas-log.jsonl` exists and includes appended entries.
...
- After completing `$run-llm-workflow`, run `python chadwin-codex/.agents/skills/chadwin-research/scripts/company_idea_queue.py remove --ticker <TICKER>` and confirm the ticker is removed from `<DATA_ROOT>/idea-screens/company-ideas-log.jsonl`.
...
- Ensure `<DATA_ROOT>/user_preferences.json` has `excluded_industries` including `biotech`.
