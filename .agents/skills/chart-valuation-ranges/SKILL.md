...
Generated files:
- `.agents/skills/chart-valuation-ranges/assets/valuation-ranges.svg`
- `.agents/skills/chart-valuation-ranges/assets/valuation-ranges.json`
...
## Selection Rules
- Enumerate all company directories under `companies/<COUNTRY>/<TICKER>`.
- Backward-compatible mode: if `--companies-root` points to a single-country folder (for example `companies/US`), treat direct children as ticker folders.
...
- `--output-dir` (default `.agents/skills/chart-valuation-ranges/assets`)
