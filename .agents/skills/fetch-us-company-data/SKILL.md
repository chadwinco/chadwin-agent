...
When present, `user_preferences.json` is respected by default for queue selection and US market guardrails.
...
- `--preferences-path "<PATH>"` overrides preferences path (default `user_preferences.json`).
