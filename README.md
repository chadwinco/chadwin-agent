...
Clone the repo, open the directory in Codex or Claude Code on your machine, and prompt it to get things setup:

```text
Let's get started
```

During setup, the agent will ask for your name and email address in order to use the SEC EDGAR API, which requires sending the name and email address of the person making the requests. This information is kept on your machine and only used for these API calls.
...
The `Chadwin Research` skill makes use of specialized helper skills to carry out specific parts of the research:
...
These skills can be accessed directly, but it's usually more powerful to use the `Chadwin Research` skill, which then knows how to invoke these skills when needed.
...
Simply invoke the `chadwin-preferences` skill with a message about your preferences. The agent will remember these preferences for all future research.
